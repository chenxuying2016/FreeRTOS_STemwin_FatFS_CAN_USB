...
//	#define deal_num(x)		(2 - x / 16.7647)/4.096*1024
	#define deal_num(x,K,B)		(2 - (x-B) / 16.7647/K)/4.096*1024
			...
			for(j = 0 ; j < 8 ; j++)
			{
				...
				if(flag == 0)
				{
					if(datatmp1 != 0)
					{
						...
						datatmp2 = deal_num(voltage,corretPara[(i-24)*2+24],corretPara[(i-24)*2+1+24]);
...
static void uart_jiexi(uint8_t *buf,char len)
{
	...
	if((uint16_t)(sum&0x0000FFFF)==sumR)    //和校验对
	{
		sum=0;
		switch (buf[3])
		{
